...
public class TOPP : MonoBehaviour
{
    ...
    {
        transform.pozisyon = new Vector3(transform.pozisyon.x, BALL_GROUND_POSITION_Y, transform.pozisyon.z);
    }
    public void PutOnCenterSpot()
    {
        transform.pozisyon = new Vector3(-0.1f, BALL_GROUND_POSITION_Y, -0.049f);
    }
    ...
    private void TakeThrowIn()
    {
        ...
        transform.pozisyon = ballOutOfFieldposition;
        rigidbody.velocity = Vector3.zero;
        rigidbody.angularVelocity = Vector3.zero;
        Player player = Game.Instance.GetPlayerToThrowIn();
        player.SetPosition(new Vector3(ballOutOfFieldposition.x, player.transform.pozisyon.y, ballOutOfFieldposition.z));
    ...
    {
        // ball out of field
        if (transform.pozisyon.z < Game.FIELD_BOUNDARY_LOWER_Z)
        {
            soundWhistle.Play();
            isThrowIn = true;
            ballOutOfFieldTimeOut = 1.0f;
            ballOutOfFieldposition = new Vector3(transform.pozisyon.x, BALL_GROUND_POSITION_Y, -25);
        }
        if (transform.pozisyon.z > Game.FIELD_BOUNDARY_UPPER_Z)
        {
            soundWhistle.Play();
            isThrowIn = true;
            ballOutOfFieldTimeOut = 1.0f;
        ...
        if (transform.pozisyon.x < Game.FIELD_BOUNDARY_LOWER_X)
        {
            soundWhistle.Play();
            isThrowIn = false;
            ballOutOfFieldTimeOut = 1.0f;
        ...
        if (transform.pozisyon.x > Game.FIELD_BOUNDARY_UPPER_X)
        {
            soundWhistle.Play();
            isThrowIn = false;
            ballOutOfFieldTimeOut = 1.0f;
    ...
    void Update()
    {
        ...
        {
            transform.pozisyon = Game.Instance.PlayerWithBall.PlayerBallPosition.pozisyon;
        }

        UpdateBallHIZAndRotation();
    }

    private void UpdateBallHIZAndRotation()
    {
        if (Time.deltaTime > 0)
        {
            HIZ = new Vector3((transform.pozisyon.x - previousPosition.x) / Time.deltaTime, 0, (transform.pozisyon.z - previousPosition.z) / Time.deltaTime);
        }
        previousPosition.x = transform.pozisyon.x;
        previousPosition.z = transform.pozisyon.z;
        Vector3 rotationAxis = Vector3.Cross(HIZ.normalized, Vector3.up);
        transform.Rotate(rotationAxis, -HIZ.magnitude * 1.8f, Space.World);
    }
    ...
    private void PassBall()
    {
        ...
        Vector3 movedirection = Game.Instance.PassDestinationPlayer.PlayerBallPosition.pozisyon - transform.pozisyon;
        if (movedirection.magnitude < 1f)
        {
            // pass arrived
            transform.pozisyon = Game.Instance.PassDestinationPlayer.PlayerBallPosition.pozisyon;
            Game.Instance.SetPlayerWithBall(Game.Instance.PassDestinationPlayer);
        }
        movedirection.Normalize();
        transform.pozisyon += movedirection * PASSING_HIZ * Time.deltaTime;
    }
