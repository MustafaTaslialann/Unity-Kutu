...
public class Game : MonoBehaviour
{
    ...
    private Player oyuncusondokunus;
    ...
    public bool WaitingForKickOff { get => bekleme; set => bekleme = value; }
    ...
    public void Awake()
    {
        ...
        gucbarı = GameObject.Find("Canvas/Panel/PowerBar");
    ...
    public Player PlayerClosestToBall(int teamNumber)
    {
        ...
        {
            float hedef = (player.transform.position - scriptBall.transform.position).magnitude;
            if(hedef < closestDistance)
            {
    ...
    public Player PlayerClosestToLocation(int teamNumber, Vector3 location)
    {
        ...
        foreach (Player player in teams[teamNumber].Players)
        {
            float hedef = (player.transform.position - location).magnitude;
            if (hedef < closestDistance)
            {
                closestDistance = hedef;
                closestPlayer = player;
            }
        }
    ...
    public void hedefgol(int team)
    {
        ...
        oyuncusondokunus.hedefgol();

        beklemesuresi(5.0f);
    }

    private void beklemesuresi(float seconds)
    {
...
//        PerformSanityChecks();
    ...
    private void DisplayErrorMessage(string mesaj)
    {
        Debug.Log("An error has occurred: " + mesaj);
    }
    ...
    public void SetMinimumDistanceOtherPlayers(Player playerToTakeDistanceFrom)
    {
        foreach (Team team in teams)
        {
            foreach (Player player in team.Players)
            {
                ...
                {
                    float hedef = (player.transform.position-playerToTakeDistanceFrom.transform.position).magnitude;
                    if (hedef < 8)
                    {
    ...
    public void SetPlayerWithBall(Player player)
    {
        ...
        if (playerWithBall != null)
        {
            ...
            player.HasBall = true;
            oyuncusondokunus = playerWithBall;
